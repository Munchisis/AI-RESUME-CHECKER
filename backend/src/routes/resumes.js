...
const { parseResume: parseStructured } = require("../services/structuredParse");
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const fallbackTitle =
      parsedSections?.basics?.name ||
      parsedSections?.basics?.title ||
      "Untitled Resume";
...
function findOriginalRange(rawText, original) {
  const escaped = original.trim().replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
...
function applyRewritesToText(rawText, rewrites) {
  ...
  for (const r of rewrites) {
    if (!r.original || !r.rewritten) continue;
    const range = findOriginalRange(result, r.original);
    if (range) {
      result =
        result.slice(0, range.start) + r.rewritten + result.slice(range.end);
    } else {
      // Fallback: append as a strengthened alternative line
      result += `\n${r.rewritten}`;
    }
